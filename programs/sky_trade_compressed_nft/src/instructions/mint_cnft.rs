...
use {
    crate::{
        ...
        state::configs::{MplBubblegum, MplTokenMetadata, SPLCompression},
    },
    anchor_lang::prelude::*,
};
...
pub fn mint_cnft(ctx: Context<MintCNft>, params: &MintCNftParams) -> Result<()> {
    ...
    if params.name.as_bytes().len() > 0 && params.name.as_bytes().len() <= NAME_LENGTH {
        ...
        return Err(SkyTradeCompressedNftError::InvalidNameLength.into());
    }

    if params.symbol.as_bytes().len() > 0 && params.symbol.as_bytes().len() <= SYMBOL_LENGTH {
    } else {
        return Err(SkyTradeCompressedNftError::InvalidSymbolLength.into());
    }

    if params.uri.as_bytes().len() > 0 && params.uri.as_bytes().len() <= URI_LENGTH {
    } else {
        return Err(SkyTradeCompressedNftError::InvalidUriLength.into());
    }
